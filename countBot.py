...
class countBot(irc.IRCClient):
    ...
    def automateStart(self):
        ...
        if ((hour != self.hourOfLastGame) and (hour > 8 and hour < 17) and ((hour % 2) == 0)):
    ...
    def adminCommands(self, message):
        if (message == self.nickname + ': quit'):
            ...
            self.userCommands('DavidS', message)
    ...
    def helpText(self):
        self.msg(self.chatroom, 'Hello Adtranies, I am countBot. My sole purpose is to spawn a quick ' +
                 'and fun counting game at 10, 12, 2, and 4. I can also be initialized by my creator, ' +
                 'DavidS. If you have any problems with me, please defer to my creator. Have a nice day :) ' +
    ...
    def privmsg(self, user, channel, message):
        if ((channel == self.chatroom) or (user.split('@')[1] == self.david)):
            try:
